...
//NotifyVerify NotifyVerify
func (m *AliPay) NotifyVerify(partnerID, notifyID string) bool {
	var param = url.Values{}
	param.Add("service", "notify_verify")
	param.Add("partner", partnerID)
	param.Add("notify_id", notifyID)
	req, err := NewRequest("GET", m.notifyVerifyDomain, param)
	if err != nil {
		return false
	}
...
//GetTradeNotification 支付结果验证
//异步通知
func (m *AliPay) GetTradeNotification(req *http.Request) (*TradeNotification, error) {
	return GetTradeNotification(req, m.AliPayPublicKey)
}

//GetTradeNotification GetTradeNotification
